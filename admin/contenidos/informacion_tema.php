...
if($_SERVER['REQUEST_METHOD']==='POST'&&$tema){
 ...
 if(!$errores)try{
  $st=$conexion->prepare("SELECT COUNT(*) FROM materias WHERE id_materia=?");$st->execute([$idMateria]);if(!(int)$st->fetchColumn())$errores[]='La materia seleccionada no existe.';$st=$conexion->prepare("SELECT COUNT(*) FROM unidades_tematicas WHERE id_unidad=? AND id_materia=? AND grado=? AND estado='Activa' LIMIT 1");$st->execute([$idUnidad,$idMateria,$grado]);if(!(int)$st->fetchColumn())$errores[]='La unidad temática no corresponde a la materia y grado seleccionados.';
