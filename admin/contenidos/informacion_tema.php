...
$errores=[];$mensajes=[];$tema=null;$materias=[];$unidades=[];
try{
 $st=$conexion->prepare("SELECT t.id_tema,t.id_materia,t.id_unidad,t.nombre,t.descripcion,t.grado,m.nombre materia,u.nombre unidad FROM temas t INNER JOIN materias m ON m.id_materia=t.id_materia LEFT JOIN unidades_tematicas u ON u.id_unidad=t.id_unidad WHERE t.id_tema=? LIMIT 1");$st->execute([$idTema]);$tema=$st->fetch(PDO::FETCH_ASSOC);
 if(!$tema){header('Location: temas.php');exit;}
 $materias=$conexion->query("SELECT id_materia,nombre FROM materias ORDER BY nombre")->fetchAll(PDO::FETCH_ASSOC);$unidades=$conexion->query("SELECT id_unidad,id_materia,grado,nombre,es_predeterminada FROM unidades_tematicas WHERE estado='Activa' ORDER BY id_materia,CAST(grado AS UNSIGNED),es_predeterminada DESC,nombre")->fetchAll(PDO::FETCH_ASSOC);
...
if($_SERVER['REQUEST_METHOD']==='POST'&&$tema){
 ...
 $nombre=trim((string)($_POST['nombre']??''));$descripcion=trim((string)($_POST['descripcion']??''));$grado=(string)($_POST['grado']??'');$idMateria=(int)($_POST['id_materia']??0);$idUnidad=(int)($_POST['id_unidad']??0);
 ...
 if($idMateria<=0)$errores[]='Selecciona una materia.';if($idUnidad<=0)$errores[]='Selecciona una unidad temática.';
 if(!$errores)try{
  $st=$conexion->prepare("SELECT COUNT(*) FROM materias WHERE id_materia=?");$st->execute([$idMateria]);if(!(int)$st->fetchColumn())$errores[]='La materia seleccionada no existe.';$st=$conexion->prepare("SELECT COUNT(*) FROM unidades_tematicas WHERE id_unidad=? AND id_materia=? AND grado=? AND estado='Activa' AND es_predeterminada=0 LIMIT 1");$st->execute([$idUnidad,$idMateria,$grado]);if(!(int)$st->fetchColumn())$errores[]='La unidad temática no corresponde a la materia y grado seleccionados o es una unidad de transición.';
  ...
  if(!$errores){$st=$conexion->prepare("UPDATE temas SET id_materia=?,id_unidad=?,nombre=?,descripcion=?,grado=? WHERE id_tema=?");$st->execute([$idMateria,$idUnidad,$nombre,$descripcion!==''?$descripcion:null,$grado,$idTema]);$st=$conexion->prepare("SELECT t.id_tema,t.id_materia,t.id_unidad,t.nombre,t.descripcion,t.grado,m.nombre materia,u.nombre unidad FROM temas t INNER JOIN materias m ON m.id_materia=t.id_materia LEFT JOIN unidades_tematicas u ON u.id_unidad=t.id_unidad WHERE t.id_tema=?");$st->execute([$idTema]);$tema=$st->fetch(PDO::FETCH_ASSOC);$mensajes[]='Los datos del tema se actualizaron correctamente.';}
...
<div class="mb-4"><label class="field-label">Unidad temática</label><select class="form-select mb-3" name="id_unidad" id="idUnidadTema" required><option value="">Selecciona una unidad</option><?php foreach($unidades as $u):?><option value="<?=$u['id_unidad']?>" data-materia="<?=$u['id_materia']?>" data-grado="<?=$u['grado']?>" data-predeterminada="<?=((int)$u['es_predeterminada']===1)?'1':'0'?>" <?=$tema['id_unidad']==$u['id_unidad']?'selected':''?>><?=e($u['nombre'])?> · <?=$u['grado']?>°</option><?php endforeach;?></select><label class="field-label">Descripción</label><textarea class="form-control" name="descripcion" rows="6" maxlength="5000" placeholder="Describe qué aprenderá el estudiante en este tema."><?=e($tema['descripcion'])?></textarea></div>
...
<script>(function(){const m=document.querySelector('[name="id_materia"]'),g=document.querySelector('[name="grado"]'),u=document.getElementById('idUnidadTema');if(!m||!g||!u)return;function f(){Array.from(u.options).forEach(o=>{if(!o.value)return;o.hidden=(m.value&&o.dataset.materia!==m.value)||(g.value&&o.dataset.grado!==g.value)||o.dataset.predeterminada==='1';});if(u.selectedOptions[0]&&u.selectedOptions[0].hidden)u.value='';}m.addEventListener('change',f);g.addEventListener('change',f);f();})();</script>
